...
/** fromPath からの相対import(importPath)を、生成ファイル一覧内のパス表記に解決する。 */
function resolveRelativeImport(fromPath: string, importPath: string): string {
  const segments = [...fromPath.split("/").slice(0, -1), ...importPath.split("/")];
...
/**
 * モデルが「componentsフォルダに分割した」と言いながら該当ファイルを生成し忘れることがある
 * (例: page.jsが./components/ChatInputをimportするのにファイル一覧に含まれない)。
 * そのままだとプレビューがModule not foundでビルドエラーになるため、事前に検出する。
 */
  ...
  for (const file of files) {
    ...
    while ((match = importRegex.exec(file.content))) {
      ...
      if (!candidates.some((c) => pathSet.has(c))) {
        missing.push(`${file.path} -> ${match[1]}`);
      }
    }
  }
  return missing;
      ...
      async () => {
        const { object } = await generateObject({
          ...
          system:
            ...
            "- 他ファイルをimportする場合、そのファイル自身も必ずfiles配列に含める" +
            "(importしているのに定義を含め忘れるとビルドエラーになるため、出力前に全importの解決先が" +
            "files配列に存在するか必ず確認する)。\n" +
        ...
        if (files.length > 0 && !files.some((f) => f.path === "app/page.js")) {
          files[0].path = "app/page.js";
        }
        return { files, missingImports: findMissingLocalImports(files) };
      },
      {
        isEmpty: (v) => v.files.length === 0 || v.missingImports.length > 0,
      }
    );
    if (files.length === 0) {
