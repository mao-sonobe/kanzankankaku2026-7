    ...
    .array(
      z.object({
        ...
        quizQuestion: z
          .string()
          .describe(
            "4択クイズの出題文。この役割の必要性を、ユーザーの企画で実現したいことの言葉で表現した質問(1〜2文、技術名は絶対に含めない)。" +
              "例: 「入力された感想を保存して、メンバーみんなで見られるようにしたい。どの技術を使う?」"
          ),
        wrongAnswers: z
          .array(
            z.object({
    ...
    .array(
      z.object({
        ...
        label: z
          .string()
          .describe(
            "この2つの技術間で実際に受け渡される『具体的なデータの中身』の説明(8〜24文字)。" +
              "企画の実データで表現し、抽象的な『APIリクエスト』『データ』は禁止。" +
...
export async function POST(req: NextRequest) {
  const { model, planText, chatHistory, currentProposal, feedback } = await req.json();
