...
/**
 * needleが本文の完全な部分文字列でない場合に備え、
 * needle内の最長の連続部分文字列で本文に実在するものを探すフォールバック。
 * (src/lib/domain/fuzzy-match.ts と同じロジックのプレーンJS移植)
 */
export function findBestSubstringMatch(haystack, needle) {
 ...
 */
export function locateNonOverlapping(haystack, items, getText) {
  const matches = [];
